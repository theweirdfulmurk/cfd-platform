...
| Реальное uptime | ~35 часов (compute + setup + smoke + buffer) |
| **Бюджет** | **~$39 (≈3 600 ₽)** ($30 + $10 запас) |
...
5. **Бюджет**: $39 (с $10 запасом) укладывается в реалистичные затраты.
...
4. Запуск `experiment/run_benchmark.py` — полный прогон 45 + 15 jobs
...
| **Total ожидаемое** | **40-43 ч** | **$44-46** |

⚠️ Из $30 budget хватит на **27 часов** — превышение **~$15**.

**Рекомендация**: положить дополнительные **$15-20** на Vast.ai счёт
перед стартом для безопасности.
...
1. **Mark**: положить $15-20 запаса на Vast.ai (опционально, для buffer)
...
6. **Я**: запуск полного 45-run эксперимента (N=16) + 15-run scaling
   (N=32 для Yaris)
7. **Я**: анализ результатов (Shapiro-Wilk, t-test, bootstrap CI),
...
- [ ] Mark кладёт $15 запаса или стартуем строго с $30?
